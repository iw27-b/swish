...
const CardsPage: React.FC = () => {
  ...
  const fetchCards = useCallback(
    ...
    [sortBy, sortOrder, priceRange.min, priceRange.max, search]
  );
  ...
  const handleSuggestionClick = (suggestion: {
    ...
    // If card suggestion with ID, navigate directly
    if (suggestion.cardId) {
      router.push(`/cards/${suggestion.cardId}`);
    }
  };
  ...
  const fetchSuggestions = useCallback(async (query: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/search/quick?query=${encodeURIComponent(
          query
        )}&limit=5&type=cards`
      );
      if (response.ok) {
        ...
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">
              The cards page encountered an error and couldn&apos;t load
              properly.
            </p>
            <button
            ...
            <div className="flex w-full">
              <div className="relative flex-1" ref={searchContainerRef}>
                ...
                <input
                  ...
                  className="pl-12 pr-4 py-3 w-full bg-gray-200 rounded-full border-none text-base placeholder-gray-400"
            ...
            <CardsContentSection
              ...
              onFilterRemove={(filterType, value) => {
                ...
                if (filterType === "organizations") {
                  newFilters.organizations = newFilters.organizations.filter(
                    (org) => org !== value
                  );
                } else if (filterType === "signed") {
                  newFilters.signed = null;
                } else if (filterType === "players") {
                  newFilters.players = newFilters.players.filter(
                    (player) => player !== value
                  );
                } else if (filterType === "seasons") {
                  newFilters.seasons = newFilters.seasons.filter(
                    (season) => season !== value
                  );
                }
